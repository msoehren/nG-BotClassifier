...
```{r}
# Recode bot variable into dummy
...
# Rename labels as Yes and No (for Bot-Yes and Bot-No)
data_base$bot <- factor(data_base$bot, levels=c(0,1), labels=c("No", "Yes"))

# Change classes of other variables
...
Count words in quirk and feedback variable

```{r}
data_clean <- data_clean %>% mutate(quirk_count = sapply(gregexpr("\\S+", data_clean$quirk), length), feedback_count = sapply(gregexpr("\\S+", data_clean$feedback), length))
```
...
# Plotting differences between variables 
...
plot_diff <- ggplot(data = df.m, aes(x = variable, y = value)) + geom_boxplot(aes(fill=bot)) + scale_y_continuous(limits = quantile(plotdata$consentTime, c(0.1, 0.9)))
```
...
# Create data partition which controls how much of the whole data should be split into train and testset
...
cm_knn <- confusionMatrix(p_knn, model_test$bot, mode = "everything", positive = "Yes")
...
cm_et <- confusionMatrix(p_et, model_test$bot, mode = "everything", positive = "Yes")
...
fin_comp <- data.frame(Model = c("XGB", "ExtraTrees", "KNN", "C5.0", "GloVe"), 
                       ...
                       Accuracy = c(cm_xgb$overall[1], cm_et$overall[1], cm_knn$overall[1], cm_c5$overall[1], eval[3]))
